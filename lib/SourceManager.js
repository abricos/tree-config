...
SourceManager.prototype.sort = function(options){
    this._isLoaded = false;
    this._orders = options || this._orders;
    ...
    // this._sources = this._sources.sort
};
...
SourceManager.prototype.add = function(sourceData){
    ...
    var sourceClasses = SourceManager._sourceClasses;
    if (!sourceClasses[sourceData.type]){
        throw new Error('Config sourceData class `' + sourceData.type + '` must be registered');
    }

    var source = this.get(sourceData.id);
    if (!source){
        source = new sourceClasses[sourceData.type](sourceData);
